----if table exists drop any foreign constraints ----
if exists(select * from dbo.sysobjects where id = object_id(N'[FX_TestRunTables_Tables]') and OBJECTPROPERTY(id, N'IsForeignKey') = 1)
...
CREATE TABLE Laboranti(
	id INT PRIMARY KEY IDENTITY(1,1),
	nume VARCHAR(100),
	echipament INT FOREIGN KEY REFERENCES Echipamente(id)
)
...
CREATE TABLE Calificare(
	...
	echipament INT FOREIGN KEY REFERENCES Echipamente(id),
	CONSTRAINT PK_LabEchipament PRIMARY KEY(laborant, echipament),
	experienta INT
)
...
----VIEW----
-- primul View: un view ce contine o comanda SELECT pe o tabela
-- Echipamentele al caror nume incepe cu litera A
CREATE OR ALTER VIEW viewEchipamente AS
	SELECT * FROM Echipamente
	WHERE denumire LIKE 'A%'
GO

-- al doilea View: un view ce contine o comanda SELECT aplicata pe cel putin doua tabele
--  Experienta laborantilor pentru echipamentele folosite
CREATE OR ALTER VIEW viewExperienta AS
	SELECT Laboranti.nume, Echipamente.denumire, Calificare.experienta
	FROM Laboranti, Echipamente, Calificare
	WHERE Calificare.laborant = Laboranti.id AND Laboranti.echipament = Echipamente.id
GO

-- al treilea View: un view ce contine o comanda SELECT aplicata pe cel putin doua tabele si avand o clauza GROUP
-- Afiseaza echipamentele folosite de cel putin un laborant
CREATE OR ALTER VIEW viewUtile AS
	SELECT E.denumire, COUNT(*) AS nr_laboranti
	FROM Laboranti L, Echipamente E
	WHERE L.echipament = E.id
	GROUP BY E.denumire
GO
...
--DELETE TestTables
EXEC AddTable @tableName='Echipamente'
EXEC AddTable @tableName='Laboranti'
EXEC AddTable @tableName='Calificare'
EXEC AddView @viewName='viewEchipamente'
...
EXEC LinkTablesAndTests @tableName='Echipamente', @testName='Test1', @noRows=2000, @position=3
...
EXEC LinkTablesAndTests @tableName='Echipamente', @testName='Test2', @noRows=20, @position=3


EXEC LinkViewsAndTests @viewName='viewUtile', @testName='Test3'
EXEC LinkViewsAndTests @viewName='viewExperienta', @testName='Test3'
EXEC LinkViewsAndTests @viewName='viewEchipamente', @testName='Test3'
...
EXEC LinkTablesAndTests @tableName='Echipamente', @testName='Test3', @noRows=10, @position=3
