...
namespace DeadlockApp
{
    class Program
    {
        ...
        static string conectionString = @"Server=PC\SQLEXPRESS;Database=Patiserie_SGBD;Integrated Security=true;TrustServerCertificate=true;";
        ...
        static bool thr1_run()
        {
            ...
            using (SqlConnection con = new SqlConnection(conectionString))
            {
                ...
                try
                {
                    ...
                    command.CommandText = "EXEC run_thread1";  // Procedura stocată pentru thread1
        ...
        private static bool thr2_run()
        {
            ...
            using (SqlConnection con = new SqlConnection(conectionString))
            {
                ...
                try
                {
                    con.Open();
                    command.Connection = con;
                    command.CommandText = "EXEC run_thread2";  // Procedura stocată pentru thread2
                    command.ExecuteNonQuery();
                    succes = true;
